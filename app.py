...
def calcul_salaire(nom, date, tarif_horaire, heure_debut, heure_fin, pause, numero_mission):
    ...
    while minute_count < worked_minutes:
        ...
        if is_nuit:
            heures_nuit += 1 / 60
        if is_dimanche and not is_nuit:
            heures_dimanche += 1 / 60
        elif is_samedi and not is_nuit:
            heures_samedi += 1 / 60
        elif not (is_nuit or is_samedi or is_dimanche):
    ...
    # Pas d'heures sup si une autre majoration est présente
